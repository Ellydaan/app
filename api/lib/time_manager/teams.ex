defmodule TimeManager.Teams do
  ...
  def remove_member(%Team{} = team, %User{} = user) do
    team_with_members = Repo.preload(team, :members)
    remaining_members = Enum.filter(team_with_members.members, &(&1.id != user.id))

    team_with_members
    |> Ecto.Changeset.change()
    |> Ecto.Changeset.put_assoc(:members, remaining_members)
    |> Repo.update()
  end
